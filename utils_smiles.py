...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root='/tmp', dataset='davis',
                 xd=None, xt_mut=None,xt_ge=None,  y=None, transform=None,
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(xd, xt_ge, xt_mut, y, smile_graph, test_drug_dict)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xd, xt_ge, xt_mut, y, smile_graph, test_drug_dict):
        assert (len(xd) == len(xt_ge) and len(xt_ge) == len(y)) and len(y) == len(
        ...
        for i in range(data_len):
            ...
            g, token_idx = smile_graph[smiles]
            ...
            if self.saliency_map == True:
                ...
                # GCNData.target_mut = torch.tensor([target_mut], dtype=torch.float, requires_grad=True)
                GCNData.target_smiles = smiles
                GCNData.token_idx = token_idx
            else:
...
def draw_AUC(pearsons, title):
    plt.figure()
    plt.plot(pearsons, label='test AUC')
