...
from build_data import construct_input_from_smiles
import random
import pickle
import sys
...
def load_drug_smile():
    ...
    for smile in drug_smile:
        ...
        g = smile_to_graph(smile)
        # smile_graph[smile] = g
        token_idx, global_label_list, atom_labels_list, atom_mask_list = construct_input_from_smiles(smile)
        smile_graph[smile] = g, token_idx
...
def save_cell_mut_matrix():
    ...
    with open('mut_dict_ccle', 'wb') as fp:
...
def save_cell_oge_matrix():
    ...
    cell_dict = {}
    i = 0
    for cell in range(1, len(elements)):
        if i < 500:
            cell_name = elements[cell].replace('"', '').replace('\n', '')
    ...
    for line in f.readlines():
        ...
        feature_names.append(elements[0])

        for i in range(1, len(elements)):
            # print(cell_names)
            cell_name = cell_names[i-1].replace('"', '').replace('\n', '')

            value = float(elements[i].replace('\n', ''))
            if min == 0:
                min = value
            if value < min:
                min = value
...
def save_mix_drug_cell_matrix(choice):
    ...
    # xem lai cai nay nao
    bExist = np.zeros((len(drug_dict), len(cell_dict_ge)))
    ...
    for item in reader:
        ...
        temp_data.append((cancer,drug, cell, ic50))
    ...
    for data in temp_data:
        # print(data)
        cancer,drug, cell, ic50 = data

        # if drug in drug_dict:
        #     print(drug)
        # if cell in cell_dict_ge:
        #     print(f'1:{cell}cell')
        # if cell in cell_dict_mut:
        #     print(cell)
        if drug in drug_dict and cell in cell_dict_ge and cell in cell_dict_mut:
            # print('-------------')
            xd.append(drug_smile[drug_dict[drug]])
            # print(f'cell_feature_mut.shape:{cell_feature_mut.shape}')
            # print(f'cell_feature_ge.shape:{cell_feature_ge.shape}')
            ...
            y.append(ic50)
            bExist[drug_dict[drug], cell_dict_ge[cell]] = 1
            
            lst_drug.append(drug)
            lst_cell.append(cell)
            lst_cancer.append(cancer)
    with open('drug_dict_ccle', 'wb') as fp:
        pickle.dump(drug_dict, fp)
    print('drug_num_all:{}'.format(len(set(lst_drug))))
    print('cell_num_all:{}'.format(len(lst_cell)))
    print('cell_num:{}'.format(len(set(lst_cell))))
    xd = np.asarray(xd)
    ...
    np.save('list_drug_mix_test_ccle', lst_drug[size1:])
    np.save('list_cell_mix_test_ccle', lst_cell[size1:])
    np.save('list_cancer_mix_test_ccle', lst_cancer[size1:])
    with open('list_drug_mix_test_ccle', 'wb') as fp:
        pickle.dump(lst_drug[size1:], fp)
        
    with open('list_cell_mix_test_ccle', 'wb') as fp:
        pickle.dump(lst_cell[size1:], fp)
    with open('list_cancer_mix_test_ccle', 'wb') as fp:
        pickle.dump(lst_cancer[size1:], fp)
    xd_train = xd[:size]
    ...
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_mix', xd=xd_train,xt_mut=xc_mut_train, xt_ge=xc_ge_train,y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_mix', xd=xd_val,xt_mut=xc_mut_val, xt_ge=xc_ge_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_mix', xd=xd_test,xt_mut=xc_mut_test, xt_ge=xc_ge_test,  y=y_test, smile_graph=smile_graph)
